...
class FCN: # Fully Connected Network
    ...
    def Op_constr(self):
        ...
        for i in list(self.Layers.keys()):
            arg = i[0]
            arg += '_' + str("H{0}".format(int(i[1])+1) in self.op_dict)
            n = int(i[1:]) + 1
    ...
    def feed_forward(self):
        ...
        for i in range (0,self.L_Count):
            ''' Note: For now we have a fix activation function for all layers. This will change latter on.'''
...
data = np.arange(600).reshape(20,6,5)
